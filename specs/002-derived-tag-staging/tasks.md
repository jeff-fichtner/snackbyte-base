...
- [ ] T001 Open the three source references side-by-side for the port: snackbyte-site's as-built
...
- [ ] T002 Confirm baseline `npm run check:all` is green on the current template before any change
...
- [ ] T003 Set `package.json` `version` to `MAJOR.MINOR` form (e.g. `1.1`) and update `package-lock.json`
      to match; the patch is no longer meaningful in the file. (FR-004)
- [ ] T004 Author `.github/workflows/ci-cd.yml` (rename from `main.yml`; delete `main.yml`): name `ci-cd`;
...
- [ ] T005 Implement the `version-and-tag` derivation in `ci-cd.yml` EXACTLY per `contracts/versioning.md`:
...
- [ ] T006 Re-run the gate: `version-and-tag` runs `check:all` on push and tags ONLY on pass (the
      authoritative backstop). (FR-011)
- [ ] T007 [P] Add `APP_VERSION` + `APP_IS_PRODUCTION` build-args to `Dockerfile` (defaults `0.0.0` /
...
- [ ] T008 [P] Author `cloudbuild.yaml`: stamp UTC build date; build forwarding
...
- [ ] T009 Update `vite.config.ts`: `__APP_VERSION__` from the `APP_VERSION` env (drop the `package.json`
...
- [ ] T010 Update `scripts/prerender.mjs` to read `APP_VERSION` and `APP_IS_PRODUCTION` **identically** to
...
- [ ] T011 [US1] Verify the first-push path in `ci-cd.yml`: no tags + non-shallow clone ⇒ mint `vMM.0`
      (main) / `vMM.0-dev` (dev), regardless of commit count (acceptance rows 1/1d). (FR-007, FR-014)
- [ ] T012 [US1] Verify the no-`dev` self-increment path: consecutive `main` pushes ⇒ `vMM.0`, `vMM.1`, …
      via global-max; CI commits nothing; `package.json` unchanged (acceptance rows 2/12; SC-001).
- [ ] T013 [US1] Verify fail-loud + shallow guard: existing target tag ⇒ fail (no deploy); shallow
...
- [ ] T014 [P] [US2] Verify `src/version.ts` reports `environment = APP_ENV ?? NODE_ENV ?? 'development'`
...
- [ ] T015 [US2] Add the `noindex` middleware to `src/server.ts`: emit `X-Robots-Tag: noindex` when
      ...
      (FR-019, NFR-003)
- [ ] T016 [P] [US2] Add Vitest coverage in `tests/` for the env label (APP_ENV=staging + NODE_ENV=production
...
- [ ] T017 [US3] Verify the reuse path in `ci-cd.yml`: a `-dev` sibling on HEAD ⇒ `main` reuses that patch,
      drops the suffix; the commit ends dual-tagged; no double-increment (acceptance row 5; SC-003).
- [ ] T018 [US3] Verify resume-direct-to-main + dev-resync + global-max-jump (acceptance rows 7/8/9): these
      are the refinement over snackbyte-site's jamming `git describe`; confirm clean advance, not a jam.
- [ ] T019 [US3] Verify prod posture: prod `/api/version` ⇒ `environment:"production"`, NO `X-Robots-Tag`;
      prod build hides the chip (defaults). (NFR-001; SC-003, SC-006 partial)
- [ ] T020 [US3] Document the **promotion gate** (`main` ⊆ `dev`, fast-forwardable) — both as the
...
- [ ] T021 [P] [US4] Add Vitest/build coverage that the staging-style build (`APP_IS_PRODUCTION=false`)
...
- [ ] T022 [US4] Confirm the chip's `display` logic in `src/web/version.ts` reads `__IS_PRODUCTION__`
...
- [ ] T023 [US5] Rewrite `DEPLOY.md` versioning sections: replace the obsoleted commit-the-bump model AND
...
- [ ] T024 [US5] Add the **branch-protection setup** to `DEPLOY.md` (a `gh api` call/procedure): require
...
- [ ] T025 [US5] Add the **corrected infra runbook** to `DEPLOY.md`: Certificate Manager **cert-map +
...
- [ ] T026 [P] [US5] Reconcile `scripts/deploy.sh` (manual no-CI deploy): it MUST NOT imply the patch
...
- [ ] T027 [US6] Rework `scripts/init.mjs`: replace the old workflow-rewrite block (which flipped
...
- [ ] T028 [US6] Update/extend the resolver's machinery tests (`tests/machinery/init.test.ts` pre-resolve)
...
- [ ] T029 [P] Principle VIII sweep: grep all shipped files (`src/`, `scripts/`, `Dockerfile`, `.github/`,
...
- [ ] T030 Run `npm run check:all` on the template; fix until green (format, lint, typecheck, all Vitest).
      (NFR-002)
- [ ] T031 [P] Update `CLAUDE.md` SPECKIT pointer if needed and confirm no stale `main.yml` references
      remain anywhere in the repo (the file is now `ci-cd.yml`).
- [ ] T032 **Fresh-app verification**: spin a throwaway app from the template and run the acceptance matrix
